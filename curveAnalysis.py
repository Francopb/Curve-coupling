...
def _reorder_equations(prb:curveCouplingProblem,
                    param:np.ndarray,
                    tol:float =1e-2) -> Tuple[np.ndarray,np.ndarray,np.ndarray]:
    """
    ...
    M_exp = np.zeros(Jn[0].shape,dtype=np.int8)
...
def _find_constant(M_exp:np.ndarray, Jn:np.ndarray, exponents)-> np.ndarray:
    """
...
def solveWithSingularities(prb: curveCouplingProblem,
    ...
    sing_outs, sing_seeds = findSingularities(prob, 20)
    sing_solutions= [solveSingularity(prob, seed, tol=tol) for seed in sing_seeds]
...
def solveWithIslands(prb: curveCouplingProblem) -> Tuple[np.ndarray, np.ndarray, List[int], List[np.ndarray]]:
    """
    ...
    _, res_brute = solveCurveCoupling_bruteForce_localSolve(prob, iter_points=20)
    for r in res_brute:
        if minDist(r)>0.01:
