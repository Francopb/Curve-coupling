...
def test_series(n: int = 2):
    ...
    for i in range(n):
        ...
        frc_vec[i] = 1.0
        err = np.dot(force_constr, frc_vec)
        assert np.sum(err**2) >= 1, "This should not be a possible force variation"

    assert np.all(disp_out==1.0), "The out disp. should be the sum of all"
    assert np.sum(force_out == 1) == 1 and np.all(force_out[np.where(force_out != 1)] == 0), "A single out force should be one and the rest zero"
